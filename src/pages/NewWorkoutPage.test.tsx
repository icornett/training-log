...
const renderPage = (): void => {
  render(
    <MemoryRouter
      initialEntries={["/training_log/1/workouts/new"]}
...
describe("NewWorkoutPage", () => {
  ...
  it("renders workout name and date fields with a Continue to Exercises button", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Continue to Exercises" }),
    ).toBeInTheDocument();
  });
  ...
  it("shows a validation error when name is empty", async () => {
    ...
    expect(
      await screen.findByText(/please enter workout name and date/i),
    ).toBeInTheDocument();
  ...
  it("saves to localStorage and navigates to /pending without calling the API", async () => {
    ...
    await waitFor(() => {
      expect(navigateMock).toHaveBeenCalledWith(
        "/training_log/1/workouts/pending",
      );
    });
