...
describe('LoginPage', () => {
  ...
  it('logs in and navigates to workouts', async () => {
    ...
    await waitFor(() => {
      expect(api.login).toHaveBeenCalledWith({ username: 'Jane Doe', password: 'valid-password' })
  ...
  it('shows API error on failed login', async () => {
    vi.mocked(api.login).mockRejectedValue(new Error('Incorrect login credentials. Please try again.'))
    ...
    expect(await screen.findByText('Incorrect login credentials. Please try again.')).toBeInTheDocument()
  })
