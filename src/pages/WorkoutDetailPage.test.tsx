...
describe('WorkoutDetailPage', () => {
  ...
  it('deletes the workout and navigates to the list', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/training_log/1/workouts/1']}>
        <Routes>
          <Route path="/training_log/:pageNumber/workouts/:workoutId" element={<WorkoutDetailPage />} />
          <Route path="/training_log/:pageNumber/workouts" element={<div>Workout List</div>} />
        </Routes>
      </MemoryRouter>,
    )
    await screen.findByRole('heading', { name: 'Upper Body' })
  ...
  it('updates an existing exercise after clicking Edit', async () => {
    const updatedWorkout: WorkoutDetails = {
      ...workoutFixture,
      exercises: [{ ...workoutFixture.exercises[0], description: 'Incline Bench Press' }],
    }
  ...
  it('shows an error when exercise deletion fails', async () => {
    vi.mocked(api.deleteExercise).mockRejectedValue(new Error('Unable to delete'))
    renderPage()
    await screen.findByRole('heading', { name: 'Upper Body' })
    await userEvent.click(screen.getByRole('button', { name: 'Delete' }))
...
describe('WorkoutDetailPage — pending mode', () => {
  ...
  const renderPendingPage = (): void => {
    render(
      <MemoryRouter initialEntries={['/training_log/1/workouts/pending']}>
        <Routes>
          <Route path="/training_log/:pageNumber/workouts/new" element={<div>New Workout Page</div>} />
          <Route
  ...
  it('shows workout name and exercise form when localStorage has pending data', async () => {
    localStorage.setItem('trainingLog:pendingWorkout', JSON.stringify({ name: 'Legs Day', date: '2026-06-01' }))
    renderPendingPage()
    expect(await screen.findByRole('heading', { name: 'Legs Day' })).toBeInTheDocument()
  ...
  it('calls createWorkoutWithExercise and navigates to the saved workout on first exercise submit', async () => {
    ...
    await waitFor(() => {
      expect(api.createWorkoutWithExercise).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ description: 'Squats', exerciseType: 'strength' }),
      )
    })
